...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if ctx.ChatSessionID != "" {
		...
		// Unified workflow for both assignment-triggered and comment-triggered runs.
		// Comment-trigger specifics (TriggerCommentID + content) are delivered via
		// the user message at the start of this turn (see buildCommentPrompt in
		// prompt.go) — NOT inlined here — to keep CLAUDE.md byte-identical across
		// trigger types and preserve prompt prefix cache on --resume.
		...
		// Reply guidance — emitted unconditionally so CLAUDE.md is byte-identical
		// across trigger types (preserves prompt prefix cache on --resume).
		// Applies only when this turn was comment-triggered; in that case the
		// user message at the start of this turn carries the trigger comment's
		// ID and content (see buildCommentPrompt in prompt.go). All numbered
		// items below are preserved verbatim from the legacy comment-triggered
		// branch so existing tests still pin the same short-phrase signals
		// (see TestInjectRuntimeConfigMentionLoopHardening).
		b.WriteString("### When this turn was triggered by a new comment\n\n")
		b.WriteString("**This turn may have been triggered by a NEW comment.** If so, the user message at the start of this turn carries the trigger comment's ID and content. Your primary job is then to respond to THAT specific comment, even if you have handled similar requests before in this session.\n\n")
		fmt.Fprintf(&b, "1. Locate the triggering comment in `multica issue comment list %s --output json` (already covered by step 2 above) — do NOT confuse it with previous comments.\n", ctx.IssueID)
		b.WriteString("   - For incremental polling, use `--since <RFC3339-timestamp>` to fetch only comments newer than a known cursor.\n")
		b.WriteString("2. **Decide whether a reply is warranted.** If you produced actual work this turn (investigated, fixed, answered a real question), post the result via step 5 above — that is a normal reply, not a noise comment. If the triggering comment was a pure acknowledgment / thanks / sign-off from another agent AND you produced no work this turn, do NOT post a reply — and do NOT post a comment saying 'No reply needed' or similar. Simply exit with no output. Silence is a valid and preferred way to end agent-to-agent conversations.\n")
		b.WriteString("3. If a reply IS warranted: do any requested work first, then **decide whether to include any `@mention` link.** The default is NO mention. Only mention when you are escalating to a human owner who is not yet involved, delegating a concrete new sub-task to another agent for the first time, or the user explicitly asked you to loop someone in. Never @mention the agent you are replying to as a thank-you or sign-off.\n")
		b.WriteString("4. **If you reply, post it as a comment — this step is mandatory when you reply.** Text in your terminal or run logs is NOT delivered to the user. **Use the trigger comment ID from the user message as `--parent`** when posting your reply.\n\n")
		// OS branch: Windows daemons must steer agents at `--content-file` because
		// PowerShell 5.1's `$OutputEncoding` defaults to ASCIIEncoding when piping
		// to native commands and silently drops non-ASCII as `?` (#2198 / #2236 /
		// #2376). Linux/macOS use HEREDOC. The trigger comment ID is kept as the
		// `<trigger-comment-id>` placeholder so this block is trigger-agnostic —
		// the agent reads the real UUID from the user message at the start of
		// the turn. OS is a daemon-instance constant, so OS-branching here does
		// NOT break the cross-trigger byte-identity invariant.
		if runtimeGOOS == "windows" {
			b.WriteString("   On Windows, write the reply body to a UTF-8 file with your file-write tool, then post it with `--content-file`. Do NOT pipe via `--content-stdin` — Windows PowerShell 5.1's `$OutputEncoding` defaults to ASCIIEncoding when piping to native commands and silently drops non-ASCII (Chinese, Japanese, Cyrillic, accents, emoji) as `?` before the bytes reach `multica.exe`.\n\n")
			b.WriteString("       # 1. Write the reply body to a UTF-8 file (e.g. reply.md) with your file-write tool.\n")
			b.WriteString("       # 2. Then run:\n")
			fmt.Fprintf(&b, "       multica issue comment add %s --parent <trigger-comment-id> --content-file ./reply.md\n\n", ctx.IssueID)
			b.WriteString("   Do NOT use inline `--content`; do NOT write literal `\\n` escapes (the file preserves real newlines). Do NOT reuse `--parent` values from previous turns in this session — always use the trigger comment ID for this turn.\n")
		} else {
			b.WriteString("   Always use `--content-stdin` with a HEREDOC, even for a single-line reply:\n\n")
			fmt.Fprintf(&b, "       cat <<'COMMENT' | multica issue comment add %s --parent <trigger-comment-id> --content-stdin\n", ctx.IssueID)
			b.WriteString("       First paragraph.\n\n       Second paragraph.\n       COMMENT\n\n")
			b.WriteString("   Do NOT use inline `--content`; do NOT write literal `\\n` escapes (the HEREDOC preserves real newlines). Do NOT reuse `--parent` values from previous turns in this session — always use the trigger comment ID for this turn.\n")
		}
